    ...
    capability: [
      {
        ...
        problem:
          "초기 MediaRecorder API로 녹음을 구현하였는데 녹음→ Blob 업로드 → 백엔드 STT 변환을 거쳐 사용자 입장에서 느리고 녹음이 완료되어야만 결과를 받을 수 있어 한계가 있었습니다.",
        strategy:
          "브라우저 내장 Web Speech API로 음성을 실시간으로 텍스트로 변환하여 AI 입력으로 전달하는 기능을 외부 라이브러리 없이 재설계하였습니다.",
        result:
          "서버 STT 처리에 대한 백엔드 의존성을 제거하였고 녹음 중에도 변환 결과를 확인, 수정 할 수 있는 인터페이스를 구현하였습니다.",
      },
      {
